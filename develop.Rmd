...
# Learning Outcomes and Purpose of the Workshop {.tabset}
...
manipulation and analysis of such data which otherwise canot be achieved
in tools such as excel.

The learning process is predicated on the model that a simple way of
learning is to
...
# RStudio Some More Detail {.tabset}
...
## The Structure of an R Markdown Document {.tabset}
...
# Conducting Analysis of Safety Data {.tabset}

## Initial Loading and Opening of Libraries {.tabset}
...
#### More Aesthetic changes to charts {.tabset}
...
Firstly.  The trend line will allways lag the real data (you can calculate centralised moving averages but this is rarely done in application and it fails to produce a current result.)
Secondly.  They can result is spurious understanding of the recent trend.  This is a result of there being  two values that impact the change from the penultimate point to the latest point.  That is, the new number entering the moving average calculation for the last point and the value exiting.
...
## Traditional 3 Moving Average {.tabset}
...
Noting that the sequence of factors is constant and arranged by the overal rank over rthe full period. As such in any one year (say 2010) the chart is not a true Pareto chart 
